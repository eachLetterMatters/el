<template lang="">
    <header class="flex justify-between items-center p-6 bg-opacity-50 relative z-20">
      <div class="text-3xl font-bold bg-linear-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">
        eachCharMatters <span class="font-black">:)</span>
      </div>
      <div class="z-30">
        <button 
          type="button"
      ...
      <nav :class="[
        ...
        bg-[#111827] md:relative md:bg-transparent md-flex md:jusitify-between 
        md:flex-row`, isMenuOpen ? 'block' : 'hidden'
      ]">
      <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0">
        <li v-for="item in Menu" :key="item.name" class="group relative">
          <a :href="item.href"
          class="relative block text-blue-100 hover:text-blue-400 transition-all
          duration-300 text-xl md:text-base font-medium group"
          @click="scrollToSection(item.href)"
          >
          {{ item.name }}
          <span class="absolute bottom-0 left-0 w-0 h-0.5 bg-blue-400 transition-all
...
const Menu = ref([
  ...
  {name:'Coś tam', href:'#testimonials'},
...
const scrollToSection = (href) => {
  ...
  if(section) {
    section.scrollInView({behaviour:'smooth'});
  }
}
